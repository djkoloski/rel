...
impl<'a, T> Slot<'a, MaybeUninit<T>> {
    /// Returns a `Slot` of the underlying `T`, casting away the `MaybeUninit`.
    pub fn uninit(self) -> Slot<'a, T> {
        // SAFETY: `MaybeUninit<T>` and `T` are guaranteed to have the same
        // layout, and so cannot alias any memory other than what it already
        // does.
        unsafe { self.cast() }
    }
}
