...
impl<T> Ref<'_, MaybeUninit<T>> {
    /// Converts to a `Ref<'a, T>`.
    ///
    /// # Safety
    ///
    /// The caller must guarantee that the value the `Ref` points to is in an
    /// initialized state.
    pub unsafe fn assume_init(this: Ref<'_, MaybeUninit<T>>) -> Ref<'_, T> {
        // SAFETY: Because the pointer used to create the `Ref` is derived from
        // another `Ref`, it is guaranteed to be non-null, properly aligned, and
        // valid for reads. It is also guaranteed to be immovable and not alias
        // any other mutable references for `'a`.
        //
        // The caller has guaranteed that the value it pointed to is
        // initialized.
        unsafe { Ref::new_unchecked(this.as_ptr().cast()) }
    }
}
