...
impl<T> Mut<'_, MaybeUninit<T>> {
    /// Converts to a `Mut<'a, T>`.
    ///
    /// # Safety
    ///
    /// The caller must guarantee that the value the `Mut` points to is in an
    /// initialized state.
    pub unsafe fn assume_init(this: Mut<'_, MaybeUninit<T>>) -> Mut<'_, T> {
        // SAFETY: Because the pointer used to create the `Mut` is derived from
        // another `Mut`, it is guaranteed to be non-null, properly aligned, and
        // valid for reads and writes. It is also guaranteed to be immovable and
        // not alias any other accessible references for `'a`.
        //
        // The caller has guaranteed that the value it pointed to is
        // initialized.
        unsafe { Mut::new_unchecked(this.as_ptr().cast()) }
    }
}
